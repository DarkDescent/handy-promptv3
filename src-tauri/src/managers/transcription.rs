use crate::audio_toolkit::{
    apply_custom_words, detect_output_language, fix_word_boundary_glue,
            ...
            let transcribe_result = catch_unwind(AssertUnwindSafe(|| -> Result<String> {
                match &mut engine {
                    LoadedEngine::TranscribeCpp(session) => {
                        ...
                        let initial_prompt = whisper_initial_prompt(&settings, model_is_whisper);
...
/// Only Whisper architectures accept the kind-tagged initial-prompt extension.
fn whisper_initial_prompt(settings: &AppSettings, model_is_whisper: bool) -> Option<String> {
    if !model_is_whisper {
        return None;
    }
    let parts = [
        settings
            ...
            .map(str::to_string),
        (!settings.custom_words.is_empty()).then(|| settings.custom_words.join(", ")),
    ];
...
fn post_process_transcription_text(
    ...
    fail_open_text_transform(raw, |raw| {
        ...
        // Legacy Breeze ASR can glue Cyrillic and mixed-script word boundaries.
        // Apply inside the fail-open transform, after custom-word correction.
        let corrected = if actual_model_id == "breeze-asr" {
...
mod tests {
    ...
    #[test]
    fn whisper_initial_prompt_respects_enabled_blank_and_custom_words() {
        let mut settings = AppSettings {
            asr_initial_prompt: "  Русский prompt  ".to_string(),
            ..Default::default()
        };
        assert_eq!(whisper_initial_prompt(&settings, true), None);
        settings.asr_prompt_enabled = true;
        assert_eq!(
            whisper_initial_prompt(&settings, true).as_deref(),
            Some("Русский prompt")
        );
        settings.custom_words = vec!["GitHub".into(), "Rust".into()];
        assert_eq!(
            whisper_initial_prompt(&settings, true).as_deref(),
            Some("Русский prompt\n\nGitHub, Rust")
        );
        assert_eq!(whisper_initial_prompt(&settings, false), None);
        settings.asr_initial_prompt = "   ".into();
        assert_eq!(
            whisper_initial_prompt(&settings, true).as_deref(),
            Some("GitHub, Rust")
        );
        settings.asr_prompt_enabled = false;
        assert_eq!(
            whisper_initial_prompt(&settings, true).as_deref(),
            Some("GitHub, Rust")
        );
    ...
    #[test]
    fn breeze_glue_runs_only_for_legacy_model_in_post_process_pipeline() {
        let mut settings = AppSettings {
            filler_word_removal_enabled: false,
            ..Default::default()
        };
        let raw = "привет.Мир test.Привет".to_string();
        settings.selected_model = "breeze-asr".into();
        ...
        assert_eq!(output, "привет. Мир test. Привет");
        settings.selected_model = "breeze-asr-25".into();
        let output = post_process_transcription_text(
            raw.clone(),
            &settings,
            &settings.selected_model,
    ...
    #[test]
    fn breeze_glue_uses_actual_model_when_stored_selection_differs() {
        let mut settings = AppSettings {
            filler_word_removal_enabled: false,
            ..Default::default()
        };
        ...
        settings.selected_model = "whisper-small".into();
        assert_eq!(
    ...
    #[test]
    fn breeze_glue_precedes_filler_removal() {
        let settings = AppSettings {
            selected_model: "breeze-asr".into(),
            filler_word_removal_enabled: true,
            ..Default::default()
        };
        let evidence = OutputLanguageEvidence::UserSelected("ru".into());
