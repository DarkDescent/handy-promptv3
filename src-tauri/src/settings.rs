...
mod tests {
    ...
    #[test]
    fn old_fork_store_salvages_invalid_field_and_preserves_custom_state() {
        let stored = serde_json::json!({
            "settings_schema_version": 0,
            "shortcut_activation": "invalid",
            "selected_model": "breeze-asr",
            "asr_prompt_enabled": true,
            "asr_initial_prompt": "Keep GitHub in Latin script",
            "custom_words": ["GitHub", "Rust"],
            "paste_method": "capglue",
            "post_process_provider_id": "custom",
            "post_process_providers": [{"id":"custom","label":"Custom","base_url":"http://localhost:11434/v1","supports_structured_output":false}],
            "post_process_models": {"custom":"gemma4:12b-mlx"},
            "post_process_api_keys": {"custom":"synthetic-w2-api-key-sentinel"},
            "post_process_prompts": [{"id":"mine","name":"Mine","prompt":"Keep ${output}"}],
            "post_process_selected_prompt_id": "mine"
        });
        let mut settings = salvage_settings(&stored);
        assert_eq!(
            settings.shortcut_activation,
            ShortcutActivation::HoldOrToggle
        );
        assert_eq!(settings.selected_model, "breeze-asr");
        assert!(settings.asr_prompt_enabled);
        assert_eq!(settings.asr_initial_prompt, "Keep GitHub in Latin script");
        assert_eq!(settings.custom_words, vec!["GitHub", "Rust"]);
        ...
        assert_eq!(
            settings
                ...
                .get("custom")
                .map(String::as_str),
            Some("synthetic-w2-api-key-sentinel")
        );
        assert!(settings.post_process_providers.iter().any(|provider| {
            provider.id == "custom" && provider.base_url == "http://localhost:11434/v1"
        }));
        assert!(settings
            .post_process_prompts
            .iter()
            .any(|prompt| { prompt.id == "mine" && prompt.prompt == "Keep ${output}" }));
        assert_eq!(
            settings
                .post_process_models
                .get("custom")
    ...
    #[test]
    fn ensure_post_process_defaults_keeps_user_renamed_promptv3() {
        let mut settings = get_default_settings();
        settings.post_process_prompts.push(LLMPrompt {
            id: LEGACY_PROMPTV3_PROMPT_ID.to_string(),
            name: "My renamed prompt".to_string(),
            prompt: LEGACY_PROMPTV3_PROMPT_TEXT.to_string(),
        });
        settings.post_process_selected_prompt_id = Some(LEGACY_PROMPTV3_PROMPT_ID.to_string());
